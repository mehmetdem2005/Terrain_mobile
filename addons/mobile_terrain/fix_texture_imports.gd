...
const NON_NORMAL_MARKERS := [
	"_diff", "_diffuse",
	"_albedo", "_basecolor", "_base_color",
	"_color", "_col",
	"_ao", "_occlusion", "_ambient_occlusion",
	"_rough", "_roughness",
...
const NORMAL_MAP_MARKERS := [
	"_norm", "_normal", "_nrm", "_normalmap", "_normal_map",
	"_nor_gl", "_nor_dx", "_nor",  # V21: AmbientCG / cc0textures convention
	"_n_ogl", "_n_dx",
]
...
func _setup_project_defaults(stats: Dictionary) -> void:
	...
	if err == OK:
		...
		var prev_str := "(önceden yoktu)" if existing_nm == null else "(önceden: %s)" % str(existing_nm)
...
func _process_import_file(import_path: String, stats: Dictionary) -> void:
	...
	if current_nm == 0 or current_nm == 1:
		...
		var reason := "dosya adında '%s' var (diffuse/non-normal göstergesi), normal_map=%d → 2" % [matched_marker, current_nm]
...
func _insert_normal_map_value(content: String, new_value: int) -> String:
	...
	return content.substr(0, line_end + 1) + ("compress/normal_map=%d\n" % new_value) + content.substr(line_end + 1)
