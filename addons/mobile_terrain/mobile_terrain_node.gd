...
func update_chunk_mesh(cx: int, cz: int):
	...
	# V20 FIX (#13): clip the outer edge of the last chunk in each axis.
	#
	# Each chunk normally has (chunk_size + 1) vertices per side so it
	# shares its outer edge with the next chunk — seamless tiling. But
	# height_data has map_size² entries indexed [0, map_size-1] per side.
	# For the last chunk (cx == num_chunks - 1), the outer vertex would
	# sit at world position map_size, which has no height entry. The old
	# code clamped that read to map_size-1, duplicating the second-to-last
	# row's height and producing a visibly flat 1-cell strip at the
	# terrain's east/south edge.
	#
	# Fix: give the last chunk one fewer vertex row in the affected
	# direction. Its outer vertex is now at index map_size-1, which has
	# valid height data and is the natural terrain boundary. The visual
	# extent of the terrain becomes (map_size-1) world units rather than
	# map_size — a 0.4% difference at the default 256, invisible in
	# practice. The seam between the last full chunk and the clipped
	# last chunk stays watertight because both sample get_height(...) at
	# the same global coordinate for the shared boundary.
	#
	# Note: this is paired with the divisibility guard in _set_map_size /
	# _set_chunk_size (#12). If map_size weren't a multiple of chunk_size,
	# clipping by one would actually make coverage worse (we'd lose the
	# last-chunk cells AND the modulo cells). The setters ensure
	# divisibility so the clip is sound.
	var num_chunks = map_size / chunk_size
	var vx_count_x = chunk_size + 1
	var vx_count_z = chunk_size + 1
	if cx == num_chunks - 1:
		vx_count_x = chunk_size
	if cz == num_chunks - 1:
		vx_count_z = chunk_size
	
	# V21 PERFORMANCE: pre-allocate the vertex arrays.
	#
	# .append() on a PackedArray re-checks capacity and may reallocate the
	# backing buffer up to log(N) times during the loop. For a chunk with
	# 33×33 = 1089 vertices, that's ~10 reallocations × 3 arrays = 30
	# reallocs per chunk. On a 1500-chunk bulk rebuild that becomes
	# 45000 unnecessary buffer copies. Resize-then-index-assign skips the
	# capacity dance entirely.
	var vx_count: int = vx_count_x * vx_count_z
	vertices.resize(vx_count)
	normals.resize(vx_count)
	uvs.resize(vx_count)
	# Precompute the UV scale factor once. Used to be inside the inner
	# loop as float(max(1, map_size - 1)) — that ran N² times, with the
	# division and max() each call. Hoisted out: 1 op per chunk.
	var inv_uv_denom: float = 1.0 / float(max(1, map_size - 1))
	
	var vi: int = 0
	# V21 PERFORMANCE: inline get_height in the hot loop.
	# get_height does a clampi(0, map_size-1) before indexing — defensively
	# correct but a measurable per-call overhead at 5K invocations per
	# chunk × ~400 chunks = 2M+ calls during a full rebuild. The center
	# read (gx, gz) is GUARANTEED in bounds (we clipped the outer chunks
	# above), so we skip clampi there entirely. Neighbour reads (hL, hR,
	# hD, hU) need bounds-handling because gx-1 / gx+1 can step outside;
	# we use clamp arithmetic inline to avoid the function-call cost.
	var max_idx: int = map_size - 1
	for z in range(vx_count_z):
		for x in range(vx_count_x):
			var gx: int = start_x + x
			var gz: int = start_z + z
			# Center vertex — guaranteed in [0, map_size-1] thanks to
			# the clipped vx_count_x/vx_count_z above.
			var center_idx: int = gz * map_size + gx
			var h: float = height_data[center_idx]
			vertices[vi] = Vector3(x, h, z)
			
			# Neighbour reads with manual clamp (avoiding the get_height
			# function call). clampi on int range is a couple of cmov
			# instructions; the GDScript function-call dispatch is ~10×
			# more expensive.
	...
	# Quads = vertices - 1 per side. Stride is the per-chunk vertex
	# count, not chunk_size+1 — the clipped chunk has a smaller stride.
	var quad_count_x: int = vx_count_x - 1
	var quad_count_z: int = vx_count_z - 1
	# V21 PERFORMANCE: pre-allocate indices array (6 indices per quad).
	...
	# V21: Provide tangents so the new PBR shader's normal maps render
	# correctly. Godot's NORMAL_MAP path needs a per-vertex (T, B, N) basis
	# to transform tangent-space normals into world space. Without
	# ARRAY_TANGENT data, Mobile renderer in particular tends to skip the
	# normal-mapping decode entirely, leaving the surface looking flat
	# (which was a major contributor to V20.1's washed-out terrain
	# appearance).
	#
	# Our UV mapping is planar (u = world_x / map_size, v = world_z /
	# map_size) so the "correct" tangent direction is +X world space,
	# bitangent is +Z. We pass T = (1, 0, 0) and let Godot's basis
	# reorthogonalisation tilt T to be perpendicular to the per-vertex
	# normal on slopes. The w component (binormal sign) is +1 because
	# our (T, B, N) ordering matches Godot's right-handed convention.
	#
	# V21 PERFORMANCE: every vertex's tangent is identical, so build the
	# array with bulk fills via resize+fill rather than four assigns per
	# vertex. Saves ~3000 array writes per chunk on a 33×33 grid.
	var v_count: int = vertices.size()
	var tangents = PackedFloat32Array()
	tangents.resize(v_count * 4)
	# Stride-write the four components separately — three of them are 0.0
	# (which resize() already filled), so we only have to touch indices
	# 4*i+0 and 4*i+3 explicitly.
	for i in range(v_count):
		var base: int = i * 4
		tangents[base] = 1.0       # T.x
		tangents[base + 3] = 1.0   # binormal sign
