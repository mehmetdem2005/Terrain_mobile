...
static func modify_height(
		...
		map_size: int,
		mark_dirty: Callable,
		cx: float, cz: float, radius: float, strength: float) -> void:
	brush.iterate_footprint(cx, cz, radius, func(x: int, z: int, f: float) -> void:
...
static func flatten_height(
		...
		map_size: int,
		mark_dirty: Callable,
		cx: float, cz: float, radius: float, target_h: float, strength: float) -> void:
	brush.iterate_footprint(cx, cz, radius, func(x: int, z: int, f: float) -> void:
...
static func smooth_height(
		...
		map_size: int,
		mark_dirty: Callable,
		cx: float, cz: float, radius: float, strength: float) -> PackedFloat32Array:
